...
- Джоба по рекомендациям друзей так же принимает на вход аргумент "какое количество километров между пользователями считать за 'находились близко рядом'". Иначе на малых выборках можно не получить соседей.
- При отладке джобы по рекомендации друзей оказалось, что в моём ODS не оказалось событий с сообщениями в каналы. Для такого случая вставил заплатку в коде (см. код джобы): форсирую в датафрейм с общими каналами несколько человек из подходящих по остальным выборкам.
...
- Используйте принципы ООП: не повторяйте код и делите его на функции — выделяйте объекты.
  ...
  - Что-то, типа конструирования повторно используемых датафреймов, вынес в отдельные функции.
  - С объектами пока я не игрался толком в питоне, не стал пока что и начинать.
...
- Ресурсы ограниченны, поэтому можете взять неполный набор данных — за конкретные даты либо выделить семпл с помощью метода sample в Spark, когда будете отлаживать скрипты и отправлять проект на проверку.
  ...
  - Джобы по заполнению ODS из STG ограничил набором sample(0.05)
  - Запустил их вручную до запуска DAG-а
  - В DAG-е их закомментировал
  - Во всех джобах параметризировал глубину для работы и т. п.
  - В DAG этим джобам прописал параметры мягче, чем надо для прод (чем в ТЗ)
...
- Промышленный код, написанный на SQL или Python с помощью PySpark, можно оформлять по известным правилам — применять логирование и тестирование.
  ...
  - всем джобам создал по две версии - для spark-submit (и соотв. для DAG-а), и для отладочного исполнения в Jupyter (файлы с постфиксом `_cell`)
  - в jupyter-notebook-коде везде отлаживают через print() и show(), в прод-коде постарался вставить использование logger-а (логирую аргументы на входе в джобу и в функции)
